...
import os as _os
CTGAN_PATH = _os.environ.get('CTGAN_PATH', _os.path.join(_os.path.dirname(__file__), '..', 'data', 'malmem_ctgan.csv'))
...
def make_local_trainable_circuit(n_qubits, L=2):
    ...
    def probs_circuit(x1, x2, w):
        ...
        for i in range(n_qubits):
            ...
            # BUGFIX (Day 1 finding): this block used to be immediately followed
            # by `qml.RY(-w[i], wires=i)`, which composes to the identity for any
            # w (RY(a).RY(b) = RY(a+b)) and made the trainable block a no-op,
            # verified numerically to ~1e-17. Removed so w genuinely persists
            # into the fidelity overlap. See circuit_fixed.py / Day1_FINAL_Bugfix_Report.md.
        for _ in range(L):
